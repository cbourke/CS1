...
(the interpreter is written in C).  However, there are some major aspects in which 
...
opening tag is \mintinline{php}{<?php} and the closing tag is \mintinline{php}{?>}.
...
of the script file being executed.  The second, \mintinline{php}{$argv} actually stores the 
arguments as strings.  We'll be able to understand the syntax later on, but
for now we can at least understand how we can access these arguments.

The variable \mintinline{php}{$argv} is an array (see Section 
\ref{chapter:php:arrays}) consisting of the command line arguments.  
To access them, you can \emph{index} them starting at zero, the first 
being \mintinline{php}{$argv[0]}, the second \mintinline{php}{$argv[1]}, etc. 
(the last one of course would be at \mintinline{php}{$argv[$argc-1]}).  
...
type.  If you assign a string to a variable, its type becomes a string.  If you
...
Internally, however, PHP does support several different types: Booleans, 
...
Floating-point numbers are also platform-dependent, but are usually
64-bit double precision numbers are defined by the \gls{ieeeLabel} 754 standard,
providing about 16 digits of precision.  

Strings and single characters are the same thing in PHP.  Strings are
...
variable identifier and the variable comes into scope.  Variable names
\emph{always} begin with a single dollar sign, \mintinline{text}{$}.
The assignment operator is a single equal sign, \mintinline{php}{=} 
and is a right-to-left assignment.  That is, the variable that we wish to 
...
a variable's name can consist of lower and uppercase alphabetic characters, 
...
sign, \mintinline{text}{$}.  Stylistically, we adopt the modern camelCasing
...
later on in the script, you use the constant's name.  You do not
...
be literals or other variables and follow the usually rules of arithmetic when 
it comes to order of precedence (multiplication 
...
as a result.  However, since PHP is dynamically typed, a variable involved in an
...
an attempt is made to convert the string variable into a numeric value by parsing
...
Consider the following examples in Code Sample \ref{code:php:typeJuggling}.
In the first block, \mintinline{php}{$a} is type juggled to the value $10$. when
added to \mintinline{php}{5}.  In the second example, \mintinline{php}{$a}
...
they are when type juggled.  However, the variables are guaranteed
...
or not \mintinline{php}{$x} is of that type.   For example:
...
\mintinline{php}{print} and \mintinline{php}{echo} (which are essentially aliases
of each other) allow you to print any variable or string.  PHP also has the
standard \mintinline{php}{printf} function to allow formatted output.  Some
examples:
...
use \mintinline{php}{fgets} (short for \textbf{f}lie \textbf{get s}tring) using the
...
\mintinline{php}{$a = trim(fgets(STDIN));}

The call to \mintinline{php}{fgets} \emph{waits} (referred to as ``blocking'')
for the user to enter input.  The user is free to start typing.  When the user i
s done, they hit the enter key at which point the program resumes and reads 
...
(integer or floating-point value for example).  A non numerical input may be 
treated as zero in the end.  The standard input is not a good mechanism 
for reading input, but it provides a good starting point.
...
which we can do by making some explicit conversion.  

We'll use a \mintinline{php}{printf} statement in the first step to prompt 
the user for input:
...
value, we can further use \mintinline{php}{floatval()}:
