...
Sometimes you may want to check that a variable falls in a \emph{range}.  For
...
similar to its usage in the inequality comparison, \mintinline{c}{!=}.
...
Historically, avoiding even a few operations in old computers meant a difference on the
...
Where \mintinline{c}{E} is a boolean expression.  If \mintinline{c}{E} evaluates to \True, the
...
To compute the amount of a tax credit and adjust the tax accordingly by using
...
\uIf{$income \geq 110000$}{
  $credit \leftarrow 0$ \;
}
\uElseIf{$numberOfChildren \leq 3$}{
  $credit \leftarrow numberOfChildren * 1000$ \;
}
\Else{
