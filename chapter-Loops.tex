...
\emph{infinite loop} \index{infinite loop}, and results in a program that never terminates.
...
As an example, consider the problem of \emph{normalizing} a number as is typically
...
\For{(initialization statement); (continuation condition); (iteration statement)}{
  Perform some action \;
}
\end{algorithm}
...
\For{$i \leftarrow 1$; $i \leq 10$; $i \leftarrow (i + 1)$}{
  Perform some action \;
}
\end{algorithm}
...
However, though December 31st, 2008 was the 266th day of the year (\mintinline{c}{days = 366})
