...
\gls{reserved word}--words or terms that are used by the programming 
...
a variable name with a number as it would make it more difficult to decipher
a program.  Yet other languages require that variables begin
with a specific character (PHP for example \emph{requires} that all variables begin
with a dollar sign, \mintinline{text}{$}).

In general, most programming languages allow you to use upper \mintinline{text}{[A-Z]}
...
\mintinline{text}{$}.  Moreover, most programming languages (like English) are 
\gls{case sensitive} \index{case sensitive} meaning that a variable name using lower case letters is not
the same variable as one that uses upper case letters.  For example, the variables
...
differences in lower and upper case letters when used in variable identifiers.  
Even in these languages, however, using a mixture of lower and upper case letters
to refer to the same variable is discouraged: it is difficult to read, inconsistent, and
just plain ugly.

Beyond the naming rules that languages may enforce, most languages have established
\glspl{naming convention}; a set of guidelines and best-practices for choosing
...
within a certain library, framework or by an organization.  Naming conventions are
...
Floating point numbers in binary are represented in a manner similar
...
be able to define the string \mintinline{c}{"Hello World"}: the double quotes are \emph{not}
...
A \gls{Boolean} is another type of variable that is used to hold a truth value, 
...
For languages that have explicit types, typical the keywords \mintinline{java}{true}
and \mintinline{java}{false} are used, but logical expressions can also be evaluated
...
Many programming languages use the keyword \mintinline{java}{null} (or sometimes
...
Another common user-defined type is an \emph{enumerated} type which allows
...
type is a \glslink{static typing}{statically typed} language.  
...
referred to as \glslink{dynamic typing}{dynamically typed} languages.  Instead
...
In a dynamically typed language, no such assumption is valid as any 
reassignment can change the variable's type.  It is impossible to enforce
this rule by the language itself and may require a lot of extra code to check
...
you first use or reference the variable.  Moreover, in some languages using 
...
some of these symbols. Usually programming languages use \mintinline{c}{*}
...
gets \glslink{truncation}{truncated} (cut-off) and only the integral part is kept for the final
...
in a statically typed language.  \glslink{type casting}{Type casting} can be
...
also be combined with non-string variables to form new strings.  You would
...
Numerical operators are no different when used in most programming languages.
...
\mintinline{c}{a += 10} is not equivalent to \mintinline{c}{a = a + 10}.  They have
...
contains a small sampling of \mintinline{c}{printf}-style supported in several languages.
...
\begin{itemize}
  ...
  \item \mintinline{c}{%f} formats a floating-point variable or literal
...
b=   3.141592653590000
\end{minted}
\caption{Result of Computation in Code Sample \ref{code:printfInCExamples1}.  Spaces are highlighted for clarity.}
