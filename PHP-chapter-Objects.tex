...
Again, we add to our example by providing two \mintinline{php}{public}
...
this is known as \gls{open recursion}.  
...
variable via its identifier \emph{but with no dollar sign}.
\footnote{You \emph{can} use the syntax \mintinline{php}{$this->$foo}
...
access or mutate (that is, change) the variables, we can define accessor 
and mutator methods (or just simply \emph{getter} and \emph{setter} 
...
The \mintinline{php}{__toString()} method is a very convenient way to
